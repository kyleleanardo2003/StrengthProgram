...
import com.example.strongman916.strongman_competition.model.OrganizerScoreRow;
...
public class HomeController {
    ...
    public String organizer(Model model) {
        try {
            ...
            model.addAttribute("configuredEvents", competitionEventRepository.findAllByOrderBySortOrderAscIdAsc());
            model.addAttribute("scoreRowsByDivision", buildScoreRowsByDivision());
        } catch (Exception e) {
    ...
    private void addResultsModel(Model model) {
        List<Competitor> competitors = loadCompetitors();
    ...
    private List<EventConfig> loadEvents() {
        return competitionEventRepository.findAllByOrderBySortOrderAscIdAsc().stream()
    ...
    private Map<String, List<OrganizerScoreRow>> buildScoreRowsByDivision() {
        ...
        for (Athlete athlete : athleteRepository.findAll()) {
            ...
            for (EventResult result : eventResultRepository.findByAthlete(athlete)) {
    ...
    private List<Competitor> loadCompetitors() {
    ...
    private void calculateDivisionScores(List<Competitor> competitors) {
        List<EventConfig> configuredEvents = loadEvents();
        for (Competitor comp : competitors) {
        ...
        competitors.sort((c1, c2) -> compareOverallStanding(c1, c2, configuredEvents));
        assignOverallPlaces(competitors, configuredEvents);
    }

    private void assignOverallPlaces(List<Competitor> competitors, List<EventConfig> configuredEvents) {
        for (int i = 0; i < competitors.size(); i++) {
            if (i > 0 && compareOverallStanding(competitors.get(i - 1), competitors.get(i), configuredEvents) == 0) {
    ...
    private int compareOverallStanding(Competitor c1, Competitor c2, List<EventConfig> configuredEvents) {
        int totalCompare = Double.compare(c2.getTotalPoints(), c1.getTotalPoints());
        if (totalCompare != 0) {
            return totalCompare;
        }

        String tiebreaker = competitionSettingsRepository.findById(1L)
                .map(settings -> settings.getOverallTiebreaker())
                .orElse("EVENT_PLACINGS");
